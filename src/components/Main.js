...
async function getQuestions() {
    await fetch("https://opentdb.com/api.php?amount=10&category=15", {
        method: "GET"
    })
        .then(res => res.json())
        .then(data => {
...
export default class Main {
    ...
    render() {
        ...
        if (this.heroModel) {
            ...
            this.margin = this.heroModel.scene.position.z / 160
            document.getElementById("player-icon").style.marginLeft = `${this.margin * 38}vw`

            if (this.heroModel.scene.position.z < -1.5) {
                this.heroModel.scene.position.z = 0
            }
            else if (this.heroModel.scene.position.z > 165) {
                this.canMove = false
